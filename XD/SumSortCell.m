...
-(id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self=[super initWithStyle:style reuseIdentifier:reuseIdentifier])
    {
        ...
        //TODO: 一种UI布局就是 一种cell 不能用同一个cell进行2中不同的 UI布局~~就算是差别非常的小
        //TODO: 一下的 全都是错的~~求破
        //设置最下面的视图
       //if([reuseIdentifier  isEqualToString:@"SumSortCell"])
        ...
        if (style==UITableViewCellStyleDefault)
        {
            ...
            //添加约束
            [self addAutoLayout];
             self.hyb_lastViewInCell = self.intro;
...
-(void)addView
{
    ...
    [self.containView addSubview:self.intro];
}
...
-(void)addAutoLayout
{
    ...
    [self.intro mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(self.iconImage.mas_bottom).offset(4*View_Radio);}];
...
{
    if (!_intro) {
        _intro=[Factory creatLabelFont:13*View_Radio Alignment:NSTextAlignmentLeft textColor:[UIColor whiteColor] bgColor:[UIColor clearColor] autoLine:YES];
    }
    return _intro;
}
